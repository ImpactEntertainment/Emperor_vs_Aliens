...
void CFrustum::CalculateFrustum(){
	...
	#ifdef RSX
	for(int a=0;a<16;a++)proj[a]=camprojectionmatrix[a];
	for(int a=0;a<16;a++)modl[a]=cammodelviewmatrix[a];
	#endif
	
	#ifdef GX
	for(int a=0;a<16;a++)proj[a]=0;
	for(int a=0;a<16;a++)modl[a]=0;
	modl[15]=1;
	for(int x=0;x<4;x++)for(int y=0;y<4;y++)
		proj[x+y*4]=perspective[x][y];
	for(int x=0;x<3;x++)for(int y=0;y<4;y++)
		modl[x+y*4]=cammatrix[x][y];
	#endif
