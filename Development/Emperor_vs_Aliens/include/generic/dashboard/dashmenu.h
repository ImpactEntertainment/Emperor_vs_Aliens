...
void dashmenu(){
	...
	y+=50;
	if(usesdl){
		if(dashbutton(x,y,300,15,1,1,(char*)"Save screenshot to SD"))dashsavescreenshot();
		y+=50;
	}
	if(dashshowframerate==0){
		if(dashbutton(x,y,300,15,1,1,(char*)"Display frame rate"))dashshowframerate=1;
		y+=50;
	}else{
		if(dashbutton(x,y,300,15,1,1,(char*)"Don't display frame rate"))dashshowframerate=0;
		y+=50;
	}
	#endif
